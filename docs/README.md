...
正在办理域名备案，通信管理局正在审核，今天是第 3 天。现在只可以通过 ip 地址（`http://8.142.32.34/`）访问 (需要关闭代理)。域名备案通过以后，可以使用 `https://www.suanfa8.com` 或者 `https://suanfa8.com` 访问。

::: danger 「算法吧」是什么？

- 这是一本小书，使用 [vuepress-theme-hope](https://vuepress-theme-hope.gitee.io/zh/) 构建，部署在阿里云服务器上（于 2022 年 1 月 16 日从 GitHub pages 迁移到 Docker 容器中运行的 nginx 服务）；
