...
- 这是一个算法与数据结构的入门教程，使用 [vuepress-theme-hope](https://vuepress-theme-hope.gitee.io/zh/) 构建，部署在阿里云服务器上（于 2022 年 1 月 16 日从 GitHub pages 迁移到 Docker 容器中运行的 nginx 服务）；
...
- 算法很有用，做算法题可以帮助我们理解算法知识点，通过面试，但是会做算法题也许没什么大用，可以看看 ACM 大佬是怎么说的：[软件工程师和算法竞赛](https://mp.weixin.qq.com/s/M7dYogVo5b-7m0ScDaGP3A)；
...
- 网站的内容比较多，最近在做格式调整，如果您发现格式太乱，影响阅读，可以在本项目的 [issue](https://github.com/liweiwei1419/liweiwei1419.github.io/issues) 留言，把链接贴一下，我看到了会处理的，感谢您的支持。
...
- 添加了「自我介绍」视频（不是面试的自我介绍指导视频），你可以在视频里看到我。可以在 [视频讲解](https://suanfa8.com/video/) 页面看到这个视频。

### 2022 年 3 月 8 日（星期二）

- 在 B 站上传了《[归并排序习题集](https://www.bilibili.com/video/BV1EF41147MG?spm_id_from=333.999.0.0)》，一共 7 个视频。

### 2022 年 2 月 7 日（星期一）

- 在「[快速排序的优化（避免递归树倾斜）](https://suanfa8.com/quick-sort/quick-sort-two-ways/)」这一节的末尾加上了「解释最后为什么交换 left 与 ge」。
...
上传了「二分查找」精讲的知识点的视频讲解，地址在 [http://8.142.32.34/binary-search/videos/01/](http://8.142.32.34/binary-search/videos/01/)，这部分视频没有在 B 站上传（因为归并排序的练习和快速排序的练习还没有录出来）。
...
- 把「二分查找」和「单调栈」的题解格式进行了进一步的修改：
  - 检查了标题的级别是否正确；
  - 粘贴力扣的题面的时候，把数据范围里出现 `104`、`105` 的地方修改成 `10^4`、`10^5` 或者 $10^4$、$10^5$。

### 2021 年 12 月 28 日至 2022 年 1 月 11 日

- 针对「专题精讲」 27 个专题的题解进行了格式的统一调整，很繁琐，消耗了很多时间；
- 在本项目 github 的 [commits](https://github.com/liweiwei1419/liweiwei1419.github.io/commits/main) 可以看到具体的提交，在这里就不重复粘贴了。
...
- 更换了「二分查找」两边夹的动画，使得「 `left` 和 `right` 向中间靠拢」的语义体现得更清晰；
